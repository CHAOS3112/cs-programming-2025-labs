...
\begin{addition}{Задание}
  ...
  Система должна учитывать реальные процессы работы заправки: продажи топлива,
контроль запасов, обслуживание цистерн, аварийные ситуации и ведение статистики.

  1. Общая идея программы Программа представляет собой консольную систему
управления автозаправочной станцией, которая позволяет:
\begin{vvsu_itemize}
...
\begin{vvsu_itemize}
  \item Обслужить клиента (касса);
  \item Проверка состояние цистерн;
  \item Оформить пополнение топлива;
  \item Баланс и статистика;
  \item История операций;
  \item Перекачка топлива между цистернами;
  \item Включение / отключение цистерн;
  \item Состояние колонок;
  \item EMERGENCY - аварийная ситуация;
  \item Выход.
\end{vvsu_itemize}
...
\begin{vvsu_itemize}
  ...
  \item Система сохранения состояния <<gas\_station\_data.json>>;
  \item Безопасность и ограничения <<min\_level>>, <<max\_level>> и <<can\_supply>>.
\end{vvsu_itemize}
...
\begin{vvsu_itemize}
  \item is\_low() — проверяет, упал ли уровень ниже порогового;
  \item disable\_if\_low() — автоматически отключает цистерну при низком уровне;
  \item can\_supply(), withdraw() — обеспечивают безопасную выдачу топлива;
  \item refill() — пополнение топлива с проверкой на переполнение;
  \item to\_dict() и from\_dict() — преобразования или преобразование объекта для сохранения состояния.
\end{vvsu_itemize}
...
\begin{vvsu_itemize}
  ...
  \item is\_available говорит, можно ли из этого сопла раздавать (если связанная цистерна включена).
\end{vvsu_itemize}
...
\subsection{Метод init\_default\_setup}
...
\begin{vvsu_itemize}
  \item save\_to\_files- сохраняет все данные АЗС (состояние цистерн, операции, статистику) в JSON-файл;
  \item load\_from\_files- загружает данные из JSON-файла при запуске.
\end{vvsu_itemize}
...
\begin{vvsu_itemize}
  \item log\_operation- добавляет запись в историю и держит историю в пределах 100 различных записей.
  \item get\_disabled\_tanks- возвращает список отключённых цистерн.
\end{vvsu_itemize}
  
  Полезные утилиты для ведения истории и проверки состояния.


% Подглава - Метод serve\_customer
\subsection{Метод serve\_customer}
...
% Подглава -check\_tanks
\subsection{Метод check\_tanks}
...
% Подглава -refill\_tank
\subsection{Методrefill\_tank}
...
% Подглава -show\_balance
\subsection{Метод show\_balance}
  Печатает показатели: число обслуженных машин, общий доход, сколько каждого топлива продано и сколько принесло денег.
...
% Подглава - transfer\_fuel
\subsection{Метод transfer\_fuel}
...
% Подглава - manage\_tanks
\subsection{Метод manage\_tanks}
...
% Подглава -show\_pumps 
\subsection{Метод show\_pumps}
...
% Подглава - show\_warning
\subsection{Метод show\_warning}
  При запуске меню показывает, если есть отключённые цистерны и почему (низкий уровень или отключили вручную).
  На рисунке \ref{fig:../code_quoting/show_warning} представлен код программы.
...
\begin{vvsu_figure}{Листинг программы для метода запуск программы}{fig:../code_quoting/start_program}
  \begin{minipage}{.75\textwidth}
...
\begin{conclusion}
\label{sec:execution}
  Разработный полностью работостпособный прототип системы управления автозаправочной станцией на языке Python продемонстрировал свою эффективность и надежность в ходе тестирования.
  Система успешно реализует основные функции, необходимые для управления АЗС, включая обслуживание клиентов, автоматическое отключение цистерн, функция пополнения и перекачки топлива межд подключенными цистернами, аварийный режим, ведение статистики продаж, а так же сохранение состояния программы.
